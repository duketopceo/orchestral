...
- **`extract`** — workers extract a JSON object per subtask; the orchestrator
  ...
  **Every declared field must be graded.** A field is graded when it is
  ...
  fabrication, and `harness.py audit` rejects the same specs as
  `absent_grading_contract`. A contract with no `required` field and no
  `expected` is rejected the same way.
